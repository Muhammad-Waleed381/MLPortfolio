...
export const MacbookScroll = ({
  ...
  const translate = useTransform(smoothScrollYProgress, [0, 1], [0, 1500]);
  const rotate = useTransform(smoothScrollYProgress, [0.15, 0.18, 0.5], [-28, -28, 0]);
  const textTransform = useTransform(smoothScrollYProgress, [0, 0.5], [0, 100]);
  const textOpacity = useTransform(smoothScrollYProgress, [0, 0.3], [1, 0]);

  return (
    <div
      ref={ref}
      className="flex min-h-[260vh] shrink-0 scale-[0.3] transform flex-col items-center justify-start py-0 [perspective:800px] sm:scale-50 md:scale-100 md:py-80"
    >
      <motion.h2
        style={{
        ...
        className="mb-20 text-center text-3xl font-semibold text-neutral-800"
      >
        {title}
      </motion.h2>
      <Lid
